    ...
    <div class="table-responsive" v-if="cartCount > 0">
        <table class="table-condensed">
           <tfoot>
                ...
                <tr v-if="shippinginfo.vat_gst_status == 1">
                  <td colspan="4"><span class="totitle">(+) VAT Charge</span></td>
                  <td>
                    <span class="total">
                      {{ shipping.vat_gst_amount + "%" }} ({{ currency.symbol }}{{ vatTotal + 0  | formatPrice }})</span
                    >
                  </td>
                </tr>
                ...
                <tr v-if="is_coupon">
                  <td colspan="4" class="text-right"><span class="totitle theme-color">Total : </span></td>
                    <td><span class="total">{{ currency.symbol }}{{ (cartTotal + vatTotal + shippingCost) | formatPrice }}</span></td>
                </tr>
                
                <tr v-if="is_coupon">
                    <td colspan="4"><span class="totitle">(-) Coupon Discount</span></td>
                    <td><span class="total">{{ currency.symbol }}{{ couponDiscount | formatPrice }}</span></td>
                </tr>
                <tr>
                    <td colspan="4" class="text-right "><span class="totitle theme-color">Grand Total : </span></td>
                    <td><span class="total">{{ currency.symbol }}{{ cartTotal + vatTotal  + Number(shippingCost) - couponDiscount | formatPrice}}</span></td>
                </tr>
                    ...
                    <input type="hidden" name="vat_total" :value="vatTotal" />
    </div>
...
<script>
  ...
  export default {
   ...
   computed : 
   {
    ...
    vatTotal() {
      if (this.shipping.vat_gst_status == 1) {
        return (
          (this.$store.getters.cart_total * Number(this.shipping.vat_gst_amount)) / 100
        );
      } else {
        return 0;
      }
